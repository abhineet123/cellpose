...
class CellposeModel:
    ...
    def _compute_masks(
        ...
        if self.device2.type == "mps" and do_3D:
            models_logger.warning(
        ...
        if changed_device_from is not None:
            models_logger.info("switching back to device %s" % self.device2)
            self.device2 = torch.device(changed_device_from)
        return masks
